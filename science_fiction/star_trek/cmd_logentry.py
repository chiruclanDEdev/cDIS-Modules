...
class cmd_logentry(cDISModule):
  ...
  def onFantasy(self, uid, chan, args):
    ...
    accountData = self.GetAccountData(userData['account'])
    channelFlags = self.GetChannelFlags(userData['account'])
    
    account = userData['account'].capitalize()
    nick = userData['nick'].capitalize()
    flags = channelFlags[chan]
